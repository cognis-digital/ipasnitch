...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:6b46c1,100:2b6cb0&height=120&section=header&text=IPASNITCH&fontSize=48&fontColor=ffffff&fontAlignY=58" width="100%" alt="IPASNITCH"/>
...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3500&pause=1000&color=6B46C1&center=true&vCenter=true&width=720&lines=Static+scanner+for+iOS+ipa+bundles+that+flags+ATS+exceptions;Self-hostable+%C2%B7+MCP-native+%C2%B7+CI-ready+%C2%B7+polyglot" width="720"/>

[![PyPI](https://img.shields.io/pypi/v/cognis-ipasnitch.svg?color=6b46c1)](https://pypi.org/project/cognis-ipasnitch/) [![CI](https://github.com/cognis-digital/ipasnitch/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/ipasnitch/actions) [![License: COCL 1.0](https://img.shields.io/badge/License-COCL%201.0-2b6cb0.svg)](LICENSE) [![Suite](https://img.shields.io/badge/Cognis-Neural%20Suite-6b46c1.svg)](https://github.com/cognis-digital)
...
Almost no good standalone CLI exists for .ipa static review on Linux CI runners; ipasnitch parses Mach-O + plist without a Mac, making iOS appsec gating possible in any pipeline.

`ipasnitch` is single-purpose, scriptable, and self-hostable: point it at a target, get prioritized results in the format your workflow already speaks (table · JSON · SARIF), gate CI on it, and let agents drive it over MCP.
...
ipasnitch scan . --fail-on high        # CI gate (non-zero exit)
```
...
```text
$ ipasnitch scan .
  [HIGH    ] IPA-001  example finding             (./src/app.py)
  [MEDIUM  ] IPA-002  another signal              (./config.yaml)
...
```mermaid
flowchart LR
  IN[target / export] --> P[ipasnitch<br/>collect + correlate]
...
`ipasnitch` is interoperable with every popular way of using AI:

- **MCP server** — `ipasnitch mcp` (Claude Desktop, Cursor, Cognis.Studio, [uncensored-fleet](https://github.com/cognis-digital/uncensored-fleet))
- **OpenAI-compatible / JSON** — pipe `ipasnitch scan . --format json` into any agent or LLM
...
*Built in the spirit of **objection / iMAS, but a non-jailbreak static analyzer in the spirit of MobSF's iOS path**, re-framed the Cognis way. Missing a credit? Open a PR.*
...
Pipes into your stack: **SARIF** for code-scanning, **JSON** for anything, an **MCP server** (`ipasnitch mcp`) for AI agents, and a webhook forwarder for SIEM/Slack/Jira. See [`docs/INTEGRATIONS.md`](docs/INTEGRATIONS.md).
...
```bash
pip install "git+https://github.com/cognis-digital/ipasnitch.git"    # pip (works today)
pipx install "git+https://github.com/cognis-digital/ipasnitch.git"   # isolated CLI
uv tool install "git+https://github.com/cognis-digital/ipasnitch.git" # uv
pip install cognis-ipasnitch                                          # PyPI (when published)
docker run --rm ghcr.io/cognis-digital/ipasnitch:latest --help        # Docker
brew install cognis-digital/tap/ipasnitch                             # Homebrew tap
curl -fsSL https://raw.githubusercontent.com/cognis-digital/ipasnitch/main/install.sh | sh
```

| Linux | macOS | Windows | Docker | Cloud |
|---|---|---|---|---|
| `scripts/setup-linux.sh` | `scripts/setup-macos.sh` | `scripts/setup-windows.ps1` | `docker run ghcr.io/cognis-digital/ipasnitch` | [DEPLOY.md](docs/DEPLOY.md) (AWS/Azure/GCP/k8s) |
...
> ### ⭐ If `ipasnitch` saved you time, **star it** — it genuinely helps others find it.

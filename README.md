...
[![PyPI](https://img.shields.io/pypi/v/cognis-ipasnitch.svg)](https://pypi.org/project/cognis-ipasnitch/)
[![CI](https://github.com/cognis-digital/ipasnitch/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/ipasnitch/actions)
...
IPASNITCH exists for one job — static scanner for ios .ipa bundles that flags ats exceptions, missing entitlements hardening, embedded urls/secrets, and weak info.plist transport settings. — and does it without a SaaS bill or heavyweight setup.
...
```bash
ipasnitch --version
ipasnitch scan .                      # scan the current project
ipasnitch scan . --format json
ipasnitch scan . --fail-on high       # non-zero exit for CI gates
ipasnitch mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **objection / iMAS, but a non-jailbreak static analyzer in the spirit of MobSF's iOS path**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`ipasnitch` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
